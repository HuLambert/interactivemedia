class Ball_Manager {
  ...
  void reset_balls(int screen_width) {
    for (int i = 0; i < balls.length; i++) {
      ...
      balls[i].vel_x = (byte)(random(-15, 15));
    }
  }
  ...
  PGraphics draw(PGraphics pg, String displaytext, color ball_color, color text_color) {
    for (int i = 0; i < balls.length; i++) {
    ...
    for (int i = 0; i < bars.bar_count(); i++) {
      for (int j = min; j < balls.length; j++) {
        ...
        if (col != Collision_Side.NONE) {
          balls[j] = handle_bar_to_ball_collision(balls[j], i, bars, col, radius);
        }

        if (balls[j].x - radius > bars.get_bar_left(i+2)) { //Gonne be honest, no fucking clue why i + 1 doesn't work, 2 seems to work
          break;
        }
      }
    }
